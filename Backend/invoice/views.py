...
class CreateInvoiceAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            result = InvoiceCalculationService.create_invoice(
                customer_id=customer_id, 
                items_data=items,
            )
            ...
            return Response({"error": "An internal error occurred during invoice creation."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)